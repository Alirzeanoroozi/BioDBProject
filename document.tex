...
Beyond primary and foreign keys, the following constraints ensure data quality:

\begin{itemize}
	\item \textbf{CHECK Constraints}: 
	\begin{itemize}
		\item \texttt{Variants.chrom\_start} $\leq$ \texttt{Variants.chrom\_end}
		\item \texttt{Transcripts.txStart} $\leq$ \texttt{Transcripts.txEnd}
		\item \texttt{Transcripts.exonCount} $\geq$ 1
		\item \texttt{Proteins.length} $>$ 0
		\item \texttt{GEO2R\_Results.adj\_p\_value} $\geq$ 0 AND $\leq$ 1
	\end{itemize}
	
	\item \textbf{UNIQUE Constraints}:
	\begin{itemize}
		\item \texttt{Genes.hgnc\_symbol} (if not NULL) must be unique
		\item \texttt{Proteins.uniprot\_id} must be unique
		\item \texttt{Transcripts.refseq\_accession} must be unique
	\end{itemize}
	
	\item \textbf{NOT NULL Constraints}: Critical identifier fields such as \texttt{Genes.gene\_stable\_id}, \texttt{Variants.refsnp\_id}, \texttt{Proteins.uniprot\_id}, and \texttt{Transcripts.refseq\_accession} are enforced as NOT NULL.
\end{itemize}

This conceptual design provides a robust foundation for integrating multi-omics data related to schwannoma, enabling complex queries that span genomic, transcriptomic, proteomic, and expression domains.
